        ...
        try {
          ...
          if (currentProgress >= 50 && !resultImage) {
            try {
              ...
              console.log(`Checking for intermediate result at ${currentProgress}%: ${resultUrl}`);
              
              // Test if the image can be loaded
              const testImage = new Image();
              testImage.onload = () => {
                console.log(`Intermediate result loaded at ${currentProgress}%`);
                setResultImage(response.data.result);
                // Don't set loading to false yet, keep showing progress
              };
              
              testImage.onerror = () => {
                console.log(`No intermediate result available yet at ${currentProgress}%`);
                // Continue waiting
              };
              
              testImage.src = resultUrl + "?t=" + new Date().getTime();
            } catch (error) {
              console.log(`Error checking intermediate result: ${error.message}`);
            }
          }
            ...
            setTimeout(async () => {
              try {
                ...
                testImage.onerror = async () => {
                  console.error("Error loading final result image, retrying...");
                  ...
                  const retryLoadImage = () => {
                    if (retryCount < maxRetries) {
                      ...
                      setTimeout(() => {
                        const newUrl = `${response.data.result}?retry=${retryCount}&t=${new Date().getTime()}`;
                      ...
                      console.error("Max retries reached. Could not load the result image.");
                      setLoading(false);
                      alert("Could not load the result image. The style transfer might have failed.");
                    }
                  };
                ...
                testImage.src = resultUrl + "?final=true&t=" + new Date().getTime();
              } catch (error) {
    ...
    <div className="style-transfer-container">
      ...
      {resultImage && (
        <div className="result-container">
          <h3>Result</h3>
          <img src={`http://localhost:5000${resultImage}`} alt="Result" className="result-image" />
          {/* Replace the anchor tag with a button that calls handleDownload */}
